...
// Tempelkan kunci sb_publishable_... milikmu di dalam tanda petik di bawah ini:
...
// Kode instansi Rutan Sinjai
const KODE_INSTANSI = "WP.23.PAS.22";

// Objek untuk menyimpan nomor urut per kombinasi kode
...
kodeUtamaSelect.addEventListener("change", function() {
  updateSubKode();
  generateNomorSurat();
});
subKodeSelect.addEventListener("change", generateNomorSurat);
jenisSuratSelect.addEventListener("change", generateNomorSurat);

// ==========================================
// 3. FUNGSI UPDATE DROPDOWN SUB-KODE
// ==========================================
...
// ==========================================
// 4. FUNGSI GENERATE NOMOR SURAT OTOMATIS
// ==========================================
function generateNomorSurat() {
  ...
  if (kodeUtama) {
    // Ambil hanya angkanya saja, misal "09.04 Cuti" menjadi "09.04"
    const angkaSubKode = subKodeValue ? subKodeValue.split(" ")[0] : "";
    
    // Kunci kombinasi: "SA.09.04"
    const kunciKombinasi = angkaSubKode ? `${kodeUtama}.${angkaSubKode}` : kodeUtama;

    // Cek nomor urut dari Google Sheets / Objek Data
    // Jika belum ada surat di kombinasi ini, maka otomatis mulai dari 1
...
// ==========================================
// 5. FUNGSI AMBIL NOMOR TERBARU DARI SUPABASE
// ==========================================
async function ambilNomorTerbaru() {
  try {
    const { data, error } = await supabaseClient
      .from('surat_keluar')
      .select('kode_utama, sub_kode, nomor_urut');

    if (error) throw error;

    const jumlahSuratPerKunci = {};
    const maxNomorPerKunci = {};

    data.forEach(item => {
      // Ambil angka sub_kode dipisah spasi (contoh: "04.05 Kenaikan Pangkat" -> "04.05")
      const angkaSub = item.sub_kode ? item.sub_kode.split(" ")[0] : "";
      const kunci = angkaSub ? `${item.kode_utama}.${angkaSub}` : item.kode_utama;

      if (kunci) {
        // 1. Hitung berapa banyak surat yang ada untuk kunci ini
        jumlahSuratPerKunci[kunci] = (jumlahSuratPerKunci[kunci] || 0) + 1;

        // 2. Ambil nilai nomor_urut jika ada
        const nilaiNomor = item.nomor_urut ? Number(item.nomor_urut) : 0;
        maxNomorPerKunci[kunci] = Math.max(maxNomorPerKunci[kunci] || 0, nilaiNomor);
      }
    });

    // Hitung nomor urut berikutnya yang siap dipakai
    for (const kunci in jumlahSuratPerKunci) {
      const berdasarkanJumlah = jumlahSuratPerKunci[kunci] + 1;
      const berdasarkanNomorUrut = maxNomorPerKunci[kunci] > 0 ? maxNomorPerKunci[kunci] + 1 : 0;

      // Pakai mana yang lebih besar antara jumlah surat + 1 atau nomor_urut terbesar + 1
      nomorUrutPerKode[kunci] = Math.max(berdasarkanJumlah, berdasarkanNomorUrut);
    }

    generateNomorSurat();
    console.log('Nomor urut terbaru berhasil diperbarui dari Supabase');

  } catch (err) {
    console.error('Gagal mengambil nomor urut:', err.message);
  }
}
...
// ==========================================
// 6. FUNGSI SIMPAN SURAT KE SUPABASE (SUBMIT FORM)
// ==========================================
form.addEventListener('submit', async (e) => {
  ...
  const pembuatSurat = document.getElementById('namaPegawai').value;
  ...
  const tembusan = document.getElementById('tembusan').value || null;
  const kodeUtama = document.getElementById('kodeUtama').value;
  const subKode = document.getElementById('subKode').value || null;
  const jenisSurat = document.getElementById('jenisSurat').value;
  const nomorSuratLengkap = document.getElementById('nomorSuratInput').value;

  const angkaSub = subKode ? subKode.split(" ")[0] : "";
  const kunciKombinasi = angkaSub ? `${kodeUtama}.${angkaSub}` : kodeUtama;
  const nomorUrutAngka = nomorUrutPerKode[kunciKombinasi] || 1;
  ...
  try {
    const { data, error } = await supabaseClient
      ...
      .insert([
        {
          pembuat_surat: document.getElementById('namaPegawai').value, // <--- Gunakan namaPegawai
          kode_utama: document.getElementById('kodeUtama').value,
          sub_kode: document.getElementById('subKode').value,
          jenis_surat: document.getElementById('jenisSurat').value,
          nomor_surat_lengkap: nomorSuratLengkap,
          tujuan_surat: document.getElementById('tujuanSurat').value,
...
// ==========================================
// 7. FUNGSI COPY NOMOR SURAT
// ==========================================
function copyNomor(teks) {
  const textToCopy = teks || document.getElementById("nomorSuratInput").value;
